function [CollectedScores,CompositeScore,CurrentEval]=OD_check_CDM(CDM,Input_Object_Designator,ODQualityThresholds)
%
...
%% === Part 0:  Input Management and Setup Initial Values ==========================================================
   ...
   % end
    ...
    %CreationTimeMatlab=datenum(strrep(CDM.Creation_time,'T',' '),'yyyy-mm-dd HH:MM:SS');
    
    % Determine the EDR bin level for the current satellite using the
    % absolute value of EDR.
    EDRBin=EDRBinDetermination(CDM.(['EDR_' Input_Object_Designator]));
    EDRBin=abs(EDRBin);
    ...
    %% CHANGE TO QUERY OBJECT TYPE FROM SPACE TRACK
    ObjectType = 1;
    % Rotating Elements

        % Get Keplerian Elements
        [KEP] = Cart2Kep([CDM.(['X_ECI_' Input_Object_Designator]),...
                          CDM.(['Y_ECI_' Input_Object_Designator]),...
                          CDM.(['Z_ECI_' Input_Object_Designator]),... 
                          CDM.(['X_DOT_ECI_' Input_Object_Designator]),... 
                          CDM.(['Y_DOT_ECI_' Input_Object_Designator]),...
                          CDM.(['Z_DOT_ECI_' Input_Object_Designator])],...
                          'MEAN','deg');
    ...
    if CDM.(['Drag_' Input_Object_Designator]) > 0
        DragEnabled=1;
    else
        DragEnabled=0;
    end
    ...
    SRPEnabled= CDM.(['SRP_' Input_Object_Designator]);
    
    
    % Build Position Covariance Matrix from CDM inputs
    PositionCovariance = [CDM.(['CovUU_' Input_Object_Designator]) CDM.(['CovUV_' Input_Object_Designator]) CDM.(['CovUW_' Input_Object_Designator])
                          CDM.(['CovUV_' Input_Object_Designator]) CDM.(['CovVV_' Input_Object_Designator]) CDM.(['CovVW_' Input_Object_Designator])
                          CDM.(['CovUW_' Input_Object_Designator]) CDM.(['CovVW_' Input_Object_Designator]) CDM.(['CovWW_' Input_Object_Designator])];
...
%% === Part I:  model settings check ================================================================
   ...
   if ~isempty(ind)
    ...
    % check to ensure that lunar-solar perturbations enabled
        CurrentEval.ModelSettings.LunarSolar= CDM.(['Lun_Sol_' Input_Object_Designator]);

    % check to ensure solid earth tides on
        CurrentEval.ModelSettings.SolidEarthTides= CDM.(['Earth_Tides_' Input_Object_Designator]);

    % check to ensure reasonability of drag value
    if EDRBin==0
        if CDM.(['Bcoef_' Input_Object_Designator])~=0
            CurrentEval.ModelSettings.DragReasonability=0;
        else
            CurrentEval.ModelSettings.DragReasonability=1;
        end
    elseif EDRBin==1
        if abs(CDM.(['Bcoef_' Input_Object_Designator]))>=ODQualityThresholds.BReasonabilityEDR1(ObjectType,1) && ...
            abs(CDM.(['Bcoef_' Input_Object_Designator]))<=ODQualityThresholds.BReasonabilityEDR1(ObjectType,2)
            CurrentEval.ModelSettings.DragReasonability=1;
        else
            CurrentEval.ModelSettings.DragReasonability=0; 
        end
    else
        if abs(CDM.(['Bcoef_' Input_Object_Designator]))>=ODQualityThresholds.BReasonabilityEDRGT1(ObjectType,1) && ...
            abs(CDM.(['Bcoef_' Input_Object_Designator]))<=ODQualityThresholds.BReasonabilityEDRGT1(ObjectType,2)
            CurrentEval.ModelSettings.DragReasonability=1;
        else
            CurrentEval.ModelSettings.DragReasonability=0; 
        end
    ...
    if EDRBin<=1
        if abs(CDM.(['SRP_' Input_Object_Designator]))>=ODQualityThresholds.SRPReasonabilityEDR01(ObjectType,1) && ...
           abs(CDM.(['SRP_' Input_Object_Designator]))<=ODQualityThresholds.SRPReasonabilityEDR01(ObjectType,2)
        ...
        if abs(CDM.(['SRP_' Input_Object_Designator]))>=ODQualityThresholds.SRPReasonabilityEDRGT1(ObjectType,1) && ...
            abs(CDM.(['SRP_' Input_Object_Designator]))<=ODQualityThresholds.SRPReasonabilityEDRGT1(ObjectType,2)
            CurrentEval.ModelSettings.SRPReasonability=1;
        else
            CurrentEval.ModelSettings.SRPReasonability=0; 
        end
...
%% === Part II:  DC Results =========================================================================
    ...
    if ~isnan(CDM.(['LUPI_' Input_Object_Designator])) && ...
            CDM.(['LUPI_' Input_Object_Designator])/upperBound<ODQualityThresholds.ODResults.LUPIRatio && ...
            lowerBound/CDM.(['LUPI_' Input_Object_Designator])<ODQualityThresholds.ODResults.MinLUPIRatio
    ...
    if ~isnan(CDM.(['DC_residuals_' Input_Object_Designator])) && ... 
            CDM.(['DC_residuals_' Input_Object_Designator])>=ODQualityThresholds.ODResults.PercentResidualAcceptance
    ...
    % type
    if ~isnan(CDM.(['WRMS_' Input_Object_Designator])) && ...
            CDM.(['WRMS_' Input_Object_Designator])<ODQualityThresholds.ODResults.WRMS(ObjectType) &&...
            CDM.(['WRMS_' Input_Object_Designator])>ODQualityThresholds.ODResults.MinWRMS(ObjectType)
...
%% === Part V:  score collected summary ===========================================================
    ...
    %score computation modified to reflect the parts not computed
    CompositeScore=(sum(CollectedScores(~isnan(CollectedScores(:,1)),1))/6*ODQualityThresholds.CompositeScoreWeightingVector(1) + ...
